...
- **AI coding agent:** Pi, installed and connected to CUNY AI Lab models via `npx @cuny-ai-lab/cail-pi` (LazyPi). Follow https://github.com/CUNY-AI-Lab/cail-pi for install steps. Gemini CLI and VS Code are no longer used in this workshop.
- **Installs:** Git (`git --version` on macOS triggers the developer tools prompt; `winget install Git.Git` on Windows) is required because LazyPi clones some packages with git. On macOS everything goes through Homebrew: `brew install node` and `brew install gh` (Homebrew's installer also brings the developer tools, so Git is already there). Windows uses `winget install OpenJS.NodeJS.LTS` and `winget install GitHub.cli`. **Do not go back to the nodejs.org installer on macOS:** it puts npm's global prefix in root-owned `/usr/local`, so LazyPi's `npm install -g @earendil-works/pi-coding-agent` fails with EACCES and LazyPi tells people to use `sudo`, which leaves root-owned files in `~/.npm` / `~/.pi`. Slides tell participants never to use sudo; the recovery is `sudo chown -R $(whoami) ~/.npm ~/.pi`, `brew install node`, `npm install -g @earendil-works/pi-coding-agent`.
